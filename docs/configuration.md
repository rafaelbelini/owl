...
|------|---------|----------|
| `.owl` | Test execution configuration | `before_all_script`, `after_all_script`, `env` |
...
If no `env` key is configured in `.owl`, Owl will automatically look for a `.env` file in the test directory:

```
tests/
├── .owl    # No env key - .env will be auto-discovered
...
Simply change the `env` value in `.owl`:
...
**Note:** If `env` is explicitly configured in `.owl`, only that file is loaded. If a different `.env` file also exists in the directory, it is ignored.
...
When running tests from a directory, the `.owl` and `.env` files in that directory apply to all tests in it and its subdirectories.

```
tests/
├── .owl   # Applied to all tests
...
For nested directories, each directory can have its own `.owl` and `.env` file. Scripts (`before_all_script`, `after_all_script`) are scoped per directory. Env files are **not** merged from parent to child directories.

```
tests/
├── .owl   # Scripts for tests/ and subdir/
├── .env         # Only for tests/ directory
├── test1.yaml
└── subdir/
    ├── .owl   # Scripts for subdir/ only
...
You can define global scripts that run before and after all tests in the `.owl` file.
...
- Scripts are executed in the directory containing the `.owl` file
...
If no `.owl` file exists in the test file's directory or any parent directory, the test runs normally with no placeholder resolution.

If the `env` key is not specified, no env file is loaded and only hardcoded values work.

### Missing Env File

If the env file specified in `.owl` doesn't exist, the test will fail with an error:
...
1. **Use `.gitignore`**: Add `.env` to `.gitignore` if it contains sensitive data. Keep `.owl` tracked as it doesn't contain secrets.
